  ...
  document.getElementById('filePayslips')?.addEventListener('change', async (e)=>{
    ...
    for(const f of files){
      try{
        ...
        tr.innerHTML = `<td>${f.name}</td><td>${row.mes||'—'}</td><td>${row.salarioBase?brl(row.salarioBase):'—'}</td><td>${row.totProventos?brl(row.totProventos):'—'}</td><td>${row.totDescontos?brl(row.totDescontos):'—'}</td><td>${row.liquido?brl(row.liquido):'—'}</td><td>${row.variaveis?brl(row.variaveis):'—'}</td><td>${row.insal?brl(row.insal):'—'}</td><td>${row.inss?brl(row.inss):'—'}</td><td>${row.ir?brl(row.ir):'—'}</td><td>${row.baseINSS?brl(row.baseINSS):'—'}</td><td>${row.baseFGTS?brl(row.baseFGTS):'—'}</td><td>${row.valorFGTS?brl(row.valorFGTS):'—'}</td><td>${row.baseIRRF?brl(row.baseIRRF):'—'}</td>`;
    ...
    const mv = document.getElementById('mediaVar'); if(mv && media>0){ mv.value = media.toFixed(2).replace('.',','); }
  });
}
...
function parsePayslipSCI(raw){
  ...
  return { mes, salarioBase, baseINSS, baseFGTS, valorFGTS, baseIRRF, insal, variaveis, totProventos, totDescontos, liquido, inss, ir };
}
